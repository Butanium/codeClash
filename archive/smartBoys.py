...
class PodController:
    ...
    def runner(self, infos: PodInfo):
        ...
        for e in enemy_pod_info:
            ...
            if dist(e.pos, infos.pos) < 2000:
                # debug("enemy close")
                speed_diff = magnitude(e.speed - infos.speed)
                # debug("magnitude diff: " + str(speed_diff))
                if ((speed_diff > 150 and scalar(e.speed, infos.speed) < 0) or speed_diff > 250) and magnitude(
                        e.speed) > 100 and magnitude(
                        infos.speed) > 100:

                    if angle_btw(e.speed, np.zeros(2, int), infos.pos) > 10 and \
                            (angle_btw(e.speed, e.pos, infos.pos) < 10 or angle_btw(infos.speed, infos.pos,
                                                                                    e.pos) < 10):
                        output_speed = "SHIELD"
                        debug("SHIELD UP")
                elif magnitude(infos.speed) > 180 and angle_btw(infos.speed, infos.pos, e.pos) < 10:
                    output_speed = "SHIELD"
                    debug("SHIELD UP")
                elif magnitude(e.speed) > 180 and angle_btw(e.speed, e.pos, infos.pos) < 10:
                    output_speed = "SHIELD"
                    debug("SHIELD UP")
        if dist(self.ally.pos, infos.pos) < 1500:
            if magnitude(infos.speed) > 0 and magnitude(self.ally.speed - infos.speed) > 100:
                if angle_btw(infos.speed, infos.pos, self.ally.pos) < 15:
                    output_speed = "SHIELD"
        self.init = True
    ...
    def fighter(self, infos: PodInfo):
        ...
        for e in enemy_pod_info:
            ...
            if dist(e.pos, infos.pos) < 2000:
                ...
                debug(e.speed)
                debug(infos.speed)
                if ((speed_diff > 150 and scalar(e.speed, infos.speed) < 0) or
                    speed_diff > 250) and magnitude(e.speed) > 100 and magnitude(
                    infos.speed) > 100:
                    if angle_btw(e.speed, np.zeros(2, int), infos.pos) > 10 and \
                            (angle_btw(e.speed, e.pos, infos.pos) < 10 or angle_btw(infos.speed, infos.pos,
                                                                                    e.pos) < 10):
